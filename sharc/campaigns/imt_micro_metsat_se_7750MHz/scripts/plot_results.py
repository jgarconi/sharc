...
# transforming dBm / MHz to dBW / 10MHz
# dBW = dBm - 30
# 10MHz = MHz + 10
...
interf_protection_criteria = {
    "Protection criterion [-148 dBW/10MHz, 20%]": [0.2, -148, "dash", "gray"],
    "Protection criterion [-127 dBW/10MHz, 0.0016%]": [0.000016, -127, "dash", "lightgray"]
}
...
def adjust_range_x(fig: go.Figure) -> go.Figure:
    ...
    if min_x_auto is not None and max_x_auto is not None:
        fig.update_layout(xaxis=dict(range=[-185, -125]))
...
plots.update_layout(
            ...
            xaxis_title="Interference Power (dBW/10MHz)",
            yaxis_title="P (I>x)",
            ...
            legend=dict(
                x=0.4,          # x position (95% from the left)
...
lim_infe = -185 #limite inferior no eixo x( e o valor que esta na adjust_range_x)
lim_supe = -125 #limite superior no eixo x( e o valor que esta na adjust_range_x)
