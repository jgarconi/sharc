...
if system_ul_interf_power_plot and system_dl_interf_power_plot:
    ...
    aggregated_plot.update_layout(
        title='CDF Plot for EESS Space Station receveid interference from Macro IMT in 7216.6 MHz',
        title_font_size=26,
        ...
        xaxis=dict(title="Interference [dBW/kHz]",title_font_size=18,tickmode="linear", dtick=5 , range=[-189,-159],tickfont=dict(size=18)), 
        legend=dict(
    ...
    aggregated_ccdf_plot.update_layout(
        title='CCDF Plot for EESS Space Station receveid interference from Macro IMT in 7216.6 MHz',
        title_font_size=26,
        yaxis=dict(title="$\\text{P } (X > x)$",title_font_size=18, tickmode="array", tickvals=ticks_at, type="log", range=[np.log10(cutoff_percentage - cutoff_percentage/2), 0],tickfont=dict(size=18)),
        xaxis=dict(title="Interference [dBW/kHz]",title_font_size=18,tickmode="linear", dtick=5 , range=[-189,-159],tickfont=dict(size=18)), 
        legend=dict(
